...
public class CollisionEvents : MonoBehaviour
{
    ...
    private void MoveSmallAsteroid(ref GameObject objAsteroid)
    {
        //Declare
       // GameObject objAsteroid = transform.GetChild(intIndex).gameObject;
        Vector3 v3Move = new Vector3(Random.Range(-1f, 1f), Random.Range(-1f, 1f), Random.Range(-1f, 1f));
        objAsteroid.GetComponent<Rigidbody>().AddForce(v3Move * 20f);
    }
    ...
    private void CreateSmallAsteroids(float fltX, float fltY, float fltZ)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            GameObject obj= (GameObject)GameObject.Instantiate(objSmallerAsteroid, v3Vector, Quaternion.identity);
            //Move this smaller asteroid to the spawn position, sometimes instantiate defaults to 0, 0, 0
            /*objSmallerAsteroid*/obj.transform.position = v3Vector;
            //Move the smaller asteroid
            MoveSmallAsteroid(ref obj);
        }
        //Set
