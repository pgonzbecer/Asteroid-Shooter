...
public class AsteroidSpawner : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            //scales asteroid randomly
            GameObject x = (GameObject)GameObject.Instantiate(asteroid, v3SpawnPositionWithinASphereShape(), Quaternion.identity, transform);
            float scale = Random.Range(.5f, 2f);
            Vector3 rndDir = new Vector3(Random.Range(-1f, 1f), Random.Range(-1f, 1f), Random.Range(-1f, 1f));

            x.transform.localScale = new Vector3(scale, scale, scale);
            x.GetComponent<Rigidbody>().AddForce(20f*rndDir);
            //Check for first one
            if (intLoop == 0)
            {
                //Move object
    ...
    private void MoveAsteroid(int intIndex)//, bool blnOffsetToPlayer = true)
    {
        ...
        //if (blnOffsetToPlayer)
        {
            ...
            v3Movement = v3OffsetPositionToPlayer(v3Movement, 12f, 16f);
        }
    ...
    private Vector3 v3OffsetPositionToPlayer(Vector3 v3Vector, float minOffset, float maxOffset)
    {
        //Declare
        float fltAmount = ((maxOffset-minOffset) * (2f * Random.value - 1f));
        Vector3 v3Offset;
        ...
        v3Offset= fltAmount * (Vector3.Cross(v3Vector, Vector3.up).normalized);
        //Return
